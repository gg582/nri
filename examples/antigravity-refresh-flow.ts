...
function parseRetryLimit(raw: string | undefined): number {
  ...
  if (!Number.isFinite(parsed) || parsed < 1) {
    throw new Error(
      `ANTIGRAVITY_MAX_ATTEMPTS must be a positive integer, got: ${raw ?? 'undefined'}`
    );
  }
  return Math.floor(parsed);
}

function assertTestSafeUrl(url: string): void {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
    throw new Error(`ANTIGRAVITY_REFRESH_URL is not a valid URL: ${url}`);
  }

  const scheme = parsed.protocol.toLowerCase();
  if (scheme !== 'http:' && scheme !== 'https:') {
    throw new Error(`ANTIGRAVITY_REFRESH_URL must use http or https: ${url}`);
  }
  ...
  if (!isTestSafe) {
    throw new Error(
      `ANTIGRAVITY_REFRESH_URL does not target a test-safe environment: ${url}. ` +
        'Allowed hostnames include localhost, 127.0.0.1, *.local, and names containing test, staging, or dev.'
    );
  }
}
...
try {
  MAX_ATTEMPTS = parseRetryLimit(process.env.ANTIGRAVITY_MAX_ATTEMPTS);
} catch (err) {
