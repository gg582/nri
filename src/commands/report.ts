...
export type ReportCommandOptions = {
  ...
  /** Applies an already approved fix in the checked-out pull-request workspace. */
...
function ensureReportOnlyDraftBranch(cwd: string): void {
  if (run("git", ["status", "--porcelain"], cwd)) {
    throw new Error("A clean working tree is required before creating a report draft pull request.");
  }

  const defaultBranch = run("gh", ["repo", "view", "--json", "defaultBranchRef", "--jq", ".defaultBranchRef.name"], cwd);
  try {
    run("git", ["fetch", "--quiet", "origin", defaultBranch], cwd);
    run("git", ["diff", "--quiet", `origin/${defaultBranch}...HEAD`], cwd);
  } catch {
    throw new Error("Draft report pull requests cannot include existing code changes. Create the draft from a branch with no code changes, or use '/report pr edit' so each fix follows the explicit approval path.");
  }
}
...
function findingsFromDiff(diff: string, cwd: string): ReportFinding[] {
  const report = generateDevelopmentReport(cwd, "pull request diff");
...
function preparePullRequestEdit(number: string, cwd: string): { branch: string; head: string } {
  if (run("git", ["status", "--porcelain"], cwd)) {
    throw new Error("A clean working tree is required before applying approved pull-request fixes.");
  }

  const branch = run("gh", ["pr", "view", number, "--json", "headRefName", "--jq", ".headRefName"], cwd);
  if (!branch) throw new Error(`Could not determine the head branch for pull request #${number}.`);

  run("gh", ["pr", "checkout", number], cwd);
  if (run("git", ["branch", "--show-current"], cwd) !== branch) {
    throw new Error(`Checked-out branch does not match pull request #${number}.`);
  }

  return { branch, head: run("git", ["rev-parse", "HEAD"], cwd) };
}

function inspectPullRequestWorkspace<T>(number: string, cwd: string, inspect: (workspace: string) => T): T {
  const head = run("gh", ["pr", "view", number, "--json", "headRefOid", "--jq", ".headRefOid"], cwd);
  if (!head) throw new Error(`Could not determine the head commit for pull request #${number}.`);

  const workspace = join(tmpdir(), `report-pr-${number}-${Date.now()}`);
  try {
    run("git", ["fetch", "--quiet", "origin", head], cwd);
    run("git", ["worktree", "add", "--detach", workspace, head], cwd);
    return inspect(workspace);
  } finally {
    try {
      run("git", ["worktree", "remove", "--force", workspace], cwd);
    } catch {
...
export async function runReportCommand(args: string[], options: ReportCommandOptions = {}): Promise<DevelopmentReport> {
  ...
  if (mode === "pr") {
    ...
    if (action === "create") {
      ensureGitHubAccess(cwd);
      ensureReportOnlyDraftBranch(cwd);
      const report = generateDevelopmentReport(cwd);
    ...
    if (action === "review") {
      const report = inspectPullRequestWorkspace(prNumber, cwd, (workspace) => {
        const diff = pullRequestDiff(prNumber, cwd);
        return {
          generatedAt: new Date().toISOString(),
          scope: `pull request #${prNumber}`,
          findings: findingsFromDiff(diff, workspace),
          approvedFixes: [],
          appliedChanges: [],
        };
      });
    ...
    if (action === "edit") {
      const target = preparePullRequestEdit(prNumber, cwd);
      const diff = pullRequestDiff(prNumber, cwd);
      const report: DevelopmentReport = {
        generatedAt: new Date().toISOString(),
        scope: `pull request #${prNumber}`,
        findings: findingsFromDiff(diff, cwd),
        approvedFixes: [],
        appliedChanges: [],
      };
      ...
      for (const finding of report.findings) {
        ...
        if (options.applyApprovedFix) {
          ...
          const after = `${run("git", ["rev-parse", "HEAD"], cwd)}\n${run("git", ["status", "--porcelain"], cwd)}`;
          if (before !== after) report.appliedChanges.push(change ?? `${finding.id}: approved changes prepared for pull request #${prNumber}.`);
        }
      }

      if (run("git", ["status", "--porcelain"], cwd)) {
        run("git", ["add", "-A"], cwd);
        run("git", ["commit", "-m", `Apply approved report fixes for #${prNumber}`], cwd);
      }

      if (run("git", ["rev-parse", "HEAD"], cwd) !== target.head) {
        if (run("git", ["branch", "--show-current"], cwd) !== target.branch) {
          throw new Error(`Refusing to push because the current branch no longer matches pull request #${prNumber}.`);
        }
        run("git", ["push"], cwd);
        report.appliedChanges.push(`Approved fixes were committed and pushed to pull request #${prNumber}.`);
      }

      run("gh", ["pr", "comment", prNumber, "--body", reportAsMarkdown(report)], cwd);
      write(`Pull request #${prNumber} was inspected. Approved changes were committed and pushed only when fixes changed the targeted pull-request branch.`);
      return report;
    }
